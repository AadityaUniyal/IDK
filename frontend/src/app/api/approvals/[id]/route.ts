...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return handle(async () => {
    ...
    const resolved = await db()`UPDATE approvals SET status = ${decision}, resolved_at = now(), resolved_by = ${user.id}
      WHERE id = ${id} AND status = 'pending' RETURNING id`;
    if (!resolved.length) return NextResponse.json({ error: "Approval was resolved by another request" }, { status: 409 });
